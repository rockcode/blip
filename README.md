...
进程；守护进程在 Claude Code 关闭后空闲自退。装法（把绝对路径换成你的）：
...
加进 `~/.claude/settings.json` 即可；末尾的 `anthropic` 换成任意目标名。
（Claude Code 插件无法直接注册状态栏，故需手动加这一行，或用随附的
`blip-hud` 插件命令半自动写入。）

> **`refreshInterval` 别省**：没有它，状态栏只在「有回复 / 切模式」等事件后
> 刷新一次，迷你图不会自己往前走。`2` = 每 2 秒刷一次（与每秒采样配合）；想更
> 跟手可设 `1`。
