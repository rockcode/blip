...
![blip 终端延迟波形 — 网络抖动](assets/screenshot-turbulence.png)

*网络抖动：延迟随四档颜色跳动（绿 / 黄 / 红），满格尖刺为超时。*

![blip 终端延迟波形 — 网络平稳](assets/screenshot-steady.png)

*网络平稳：波形贴底、整体偏绿。*

## 由来

用 AI 的时候网络总不稳定——回答忽快忽慢、时不时卡住或超时，搞不清到底是某家大模型 API 在抽风、还是自己的网出了问题，于是老得切到终端 `ping` 一下、确认网络状况。来回折腾实在烦，干脆写了这个常驻终端的小工具：把本机到各家大模型 API 的延迟实时画成滚动的「电波图」，谁快谁慢、有没有丢包、网络稳不稳，扫一眼就知道，不用再手动一个个去试。
